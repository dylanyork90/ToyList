...
let selectedGenre = "not selected";  // for my dropdown list of toy genre
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("buttonAdd").addEventListener("click", function () {
    let newToy = new ToyObject(document.getElementById("title").value, document.getElementById("year").value,
       selectedGenre, document.getElementById("creator").value, document.getElementById("height").value);
       addNewToy(newToy); // now post new toy object to node server
    });
  ...
  document.getElementById("buttonDelete").addEventListener("click", function () {
    ...
    fetch('users/deleteToy/' + deleteTitle , {
    // users/deleteToy/Moonstruck   for example, this is what the URL looks like sent over the network
        method: 'DELETE'
    })  
...
$(document).on("pagebeforeshow", "#detailPage", function (event) {   // have to use jQuery 
    ...
    for(let i=0; i < toyArray.length; i++) {   
        if(toyArray[i].Title == localTitle){
            document.getElementById("oneTitle").innerHTML =  toyArray[i].Title;
            document.getElementById("oneYear").innerHTML =  toyArray[i].Year;
            document.getElementById("oneGenre").innerHTML =  toyArray[i].Genre;
            document.getElementById("oneHeight").innerHTML =   toyArray[i].Height;
            document.getElementById("oneCreator").innerHTML =   toyArray[i].Creator;
            break;
        }  
    }
 });
...
{
  // clear prior data
  var divToyList = document.getElementById("divToyList");
  while (divToyList.firstChild) {    // remove any old data so don't get duplicates
  divToyList.removeChild(divToyList.firstChild);
  };

  var ul = document.createElement('ul');  
  toyArray.forEach(function (element,) {   // use handy array forEach method
    var li = document.createElement('li');
    li.innerHTML = "<a data-transition='pop' class='oneToy' data-parm=" + element.Title + "  href='#detailPage'>Get Details </a> "  + element.Title + "  " + element.Genre;
...
function compareTitle(a, b) {
    ...
    const toyA = a.Title.toUpperCase();
    const toyB = b.Title.toUpperCase();
...
function compareGenre(a, b) {
    // Use toUpperCase() to ignore character casing
    const toyA = a.Genre.toUpperCase();
    const toyB = b.Genre.toUpperCase();
...
function FillArrayFromServer(){
    ...
    toyArray.length = 0;  // clear array
    toyArray = serverData;   // use our server json data which matches our objects in the array perfectly
...
function addNewToy(newToy){
    ...
    // the required post body data is our toy object passed in, newToy
    
    // create request object
    const request = new Request('/users/addToy', {
        method: 'POST',
